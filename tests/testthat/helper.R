...
matrix_for_pca_no_outliers <- function() {
  ...
  # 50 samples with 20 genes expressed at ~1, 20 at ~5, and 20 at ~10
  matrix(c(stats::rnorm(n_samples*20, mean = 1, sd = 0.5),
           stats::rnorm(n_samples*20, mean = 5, sd = 1),
           stats::rnorm(n_samples*20, mean = 10, sd = 2)),
         ncol = n_samples, byrow = TRUE,
         dimnames = list(paste0("gene", 1:60), paste0("sample", 1:n_samples)))
}
...
matrix_for_pca_with_outliers <- function() {
  ...
  cbind(
    data,
    matrix(data[, 1:2] * c(rep(1.5, 30), rep(0.5, 30)),
           ncol = n_samples, byrow = TRUE,
...
matrix_for_pca_with_groups <- function() {
  ...
  data3 <- matrix_for_pca_with_outliers() + 10
  ...
  data3 <- data3[, c(51:52, 1:50)]
  colnames(data3) <- c("group3_sample_out3", "group3_sample_out4", paste0("group3_sample", 3:52))
